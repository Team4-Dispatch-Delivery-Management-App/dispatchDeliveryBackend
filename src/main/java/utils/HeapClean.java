...
public @Data class HeapClean {
	...
	public void check(DateTime currentTime) {
		...
		while (currentTime.isBefore((ReadableInstant) departpq.peek())) {
		...
		while (currentTime.isBefore((ReadableInstant) userpq.peek())) {
		...
		while (currentTime.isBefore((ReadableInstant) carrierpq.peek())) {
